class SidenavResolver
  ...
  def directories(path, name = nil)
    ...
    Dir.foreach(path) do |entry|
      ...
      if File.directory?(full_path)
        ...
        doc_path = DocFinder.find(root: @path, document: full_path, language: @language, strip_root_and_language: true).path
  ...
  def document_meta(item)
    doc = DocFinder.find(root: item[:root] || @path, document: item[:path], language: @language, strip_root_and_language: true).path
    YAML.load_file(doc)
  end
